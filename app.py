...
# Pipeline tanımları (create_features, build_prep_pipeline) artık model_utils.py'de.
# train.py'nin kaydettiği model.pkl'i joblib ile yüklerken, pickle içindeki
# FunctionTransformer'ın referans verdiği create_features fonksiyonunu Python'ın
# bulabilmesi için, bu fonksiyon her iki dosyada da AYNI modülden (model_utils)
# import edilmiş olmalı — aksi halde "Can't get attribute" hatası alınır.
...
# ==============================================================================
# SIDEBAR — veri yükleme (opsiyonel, sadece EDA/model karşılaştırma için gerekli)
# ==============================================================================
import os
import joblib
...
uploaded = st.sidebar.file_uploader(
    "Housing.csv yükle (opsiyonel — sadece Keşifçi Analiz/Model Karşılaştırma/Hata Analizi için gerekir)",
    type="csv"
)

# Veri var mı: kullanıcı yüklediyse ya da repo'da Housing.csv varsa
...
# Önceden eğitilmiş model var mı — Tahmin sekmesi CSV'ye ihtiyaç duymadan bunu kullanır
...
# ==============================================================================
# TAB 1 — EDA (veri gerektirir)
# ==============================================================================
with tab1:
    if not data_available:
        st.info("Bu sekme için Housing.csv gerekli. Soldaki menüden yükleyebilirsin.")
    else:
...
# ==============================================================================
# TAB 2 — Model karşılaştırma (veri gerektirir)
# ==============================================================================
with tab2:
    if not data_available:
        st.info("Bu sekme için Housing.csv gerekli. Soldaki menüden yükleyebilirsin.")
...
# ==============================================================================
# TAB 3 — Hata analizi (veri gerektirir)
# ==============================================================================
with tab3:
    if not data_available:
        st.info("Bu sekme için Housing.csv gerekli. Soldaki menüden yükleyebilirsin.")
...
# ------------------------------------------------------------------------------
# ÖNCELİK SIRASI:
#   1) model.pkl varsa (train.py ile önceden eğitilmiş) -> onu kullan, CSV gerekmez
#   2) yoksa ama Housing.csv varsa -> anlık eğit (yavaş ama fallback olarak çalışır)
#   3) ikisi de yoksa -> kullanıcıyı bilgilendir
# ==============================================================================
with tab4:
    ...
    if model_available:
        ...
        st.caption("Kullanılan model: önceden eğitilmiş model.pkl")
    elif data_available:
        with st.spinner("Kayıtlı model bulunamadı, veriyle anlık eğitiliyor..."):
            _, fitted_pipelines, best_name = train_all_models(X_train, y_train_model, X_test, y_test_model)
        active_pipeline = fitted_pipelines[best_name]
        st.caption(f"Kullanılan model: {best_name} (anlık eğitildi)")
    else:
        st.warning(
            "Ne model.pkl ne de Housing.csv bulunamadı. Tahmin yapabilmek için ikisinden "
            "birini repo'ya eklemen ya da soldaki menüden CSV yüklemen gerekiyor."
        )
